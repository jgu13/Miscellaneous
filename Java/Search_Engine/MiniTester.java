...
public class MiniTester{
 ...
 private int testAssignRanks(int testIdx)
 {
  ...
  try {
   ...
   for (String v : vertices) {
       double actualRank = searchEngine.internet.getPageRank(v);
    if (actualRank <= 0) {
     comment = comment + " Page rank for vertex " + v + " is negetive. Page rank should be positive at each iteration." + "\n";
     result = false;
    }
        double expectedRank = searchEngine.parser.getPageRank(v);
    if(searchEngine.internet.getPageRank(v) != expectedRank){
        comment = comment + "Page rank for vertex " + v + "is incorrect. Expected " + expectedRank + " Actual " + actualRank + "\n";
    }
   }
 ...
 private int testGetResults0(int testIdx)  //Test the final result. Uses testAcyclic.xml
 {
  ...
  try {
   ...
   if(results != null && results.get(0).equals("siteE")){ //Make sure the most relevant url is correct
    grade=5;
   }
   else{
