...
public class DogShelter implements Iterable<Dog> {
	...
	public class DogNode {
		...
		public DogNode adopt(Dog d) {
			...
			DogNode start = root;  //which node to start downHeap
			root = remove(d, root, start);  //remove the key dog, also find the starting root
			downHeap(start);  //perform downHeap from the start node
            return root;
		}
		...
		private DogNode remove(Dog key, DogNode root, DogNode start){
			if(root == null) return root;
			else if(key.compareTo(root.d) < 0){  //if the key is smaller than root
				root.younger = remove(key, root.younger, start);
			}else if(key.compareTo(root.d) > 0){  //if the key is larger than the root
				root.older = remove(key, root.older, start);
			}else{  //root.d == key
				if(root.younger == null){
					start =root.older;
					if(root.older!=null)root.older.parent = root.parent;
					return root.older;
				}
				else if(root.older == null){
					start = root.younger;
					if(root.younger!=null)root.younger.parent = root.parent;
					return root.younger;
				}
				else{
					//root has both left and right child
					Dog oldD = (root.younger).findOldest();  //find the oldest of left subtree
					root.d = oldD; //change the element to oldD, not changing its parent
					start = root;
					root.younger = remove(root.d, root.younger, start);   //remove the duplicated leaf node
				}
			}
			return root;
		}
		private void downHeap(DogNode root){
			while(root.older != null) {  //if we have left node
				DogNode child = root.older;
				//if the left node has days more than the right node
				if (root.younger != null && compareDays(root.younger.d, root.older.d) > 0) {
					child = root.younger;
				}
				...
				} else break;  //already have the longest days on top
			}
		}
	...
	public static void testAdopt(DogNode root){
		root = root.adopt(root.d);
